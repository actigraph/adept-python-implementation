...
## that is corresponding to time of stride start for each element of
...
results_partial_dir = os.path.join(project_dir_path, "data_results/20190805_DDV001A2302/results_partial")
## Desired directory of device wear information that corresponds to
## pattern segmentation results
results_wear_dir = os.path.join(project_dir_path, "data_results/20190805_DDV001A2302/results_wear")
...
def wrapper_compute_results_wear(subj_fname_prefix):
    ...
    res_fname_list = [val for val in os.listdir(results_partial_dir) if subj_fname_prefix in val]
    ...
    for res_fname in res_fname_list:
        ...
        try:
            ...
            hdf5_wear_fname = "DDV001A2302" + "_" + res_fname.split("_")[1] + ".wearDetection"
            ...
            is_worn = [int(daily_wear[int(val)]) for val in res_f[:, 0]]
            is_worn_arr = np.array(is_worn)
